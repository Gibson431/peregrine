    ...
    SI: SensorInterface,
{
    ...
    pub fn new(sensor_interface: SI) -> Result<BMP280<SI>, SI::SensorError> {
        let mut chip = BMP280::<SI> {
            sensor_interface,
            dig_t1: 0,
            dig_t2: 0,
            dig_t3: 0,
    ...
    fn read_calibration(&mut self) -> Result<(), SI::SensorError> {
        let mut data: [u8; 24] = [0; 24];
        let _ = self
            .sensor_interface
            .write_read(&[Register::calib00 as u8], &mut data)?;
    ...
    pub fn pressure(&mut self) -> Result<f64, SI::SensorError> {
        let mut data: [u8; 6] = [0; 6];
        let _ = self
            .sensor_interface
            .write_read(&[Register::press as u8], &mut data)?;
    ...
    pub fn pressure_one_shot(&mut self) -> Result<f64, SI::SensorError> {
    ...
    pub fn temp(&mut self) -> Result<f64, SI::SensorError> {
        let mut data: [u8; 6] = [0; 6];
        let _ = self
            .sensor_interface
            .write_read(&[Register::press as u8], &mut data)?;
    ...
    pub fn config(&mut self) -> Result<Config, SI::SensorError> {
    ...
    /// Sets configuration
    pub fn set_config(&mut self, new: Config) -> Result<(), SI::SensorError> {
        let config: u8 = 0x00;
    ...
    /// Sets control
    pub fn set_control(&mut self, new: Control) -> Result<(), SI::SensorError> {
    ...
    /// Returns control
    pub fn control(&mut self) -> Result<Control, SI::SensorError> {
    ...
    pub fn status(&mut self) -> Result<Status, SI::SensorError> {
    ...
    /// Returns device id
    pub fn id(&mut self) -> Result<u8, SI::SensorError> {
        self.read_byte(Register::id)
    }

    /// Software reset, emulates POR
    pub fn reset(&mut self) -> Result<(), SI::SensorError> {
        self.write_byte(Register::reset, 0xB6)?; // Magic from documentation

        Ok(())
    }

    fn write_byte(&mut self, reg: Register, byte: u8) -> Result<(), SI::SensorError> {
        let mut buffer = [0];
        let _ = self
            .sensor_interface
    ...
    fn read_byte(&mut self, reg: Register) -> Result<u8, SI::SensorError> {
        let mut data: [u8; 1] = [0];
        let _ = self.sensor_interface.write_read(&[reg as u8], &mut data)?;
